...
3. **Mock Only Uncontrollable** - Use real Redis in Docker where possible. Don't mock what you can control - like the data content.
4. **Seed Data with Reset** - Reset Redis to known state before each test if needed.
...
- `tests/e2e/fixtures/` - Auth and Redis fixtures
...
4. **Update seed data** - Add any new test data to Redis fixtures
...
- **Redis**: Real Redis in Docker for session and user data storage
